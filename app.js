...
function calcPlanGivenQuantities() {
  ...
  foods.forEach((food) => {
    ...
    totals.energy += q * food.portionFactor * food.energy;
...
function renderFoodCard(food) {
  ...
  card.innerHTML = `
    ...
    <div class="price-row">
      <label class="price-label" for="price-${food.id}">Giá (VNĐ/${food.id === 'egg' ? 'quả' : '100g'})</label>
...
function computeOptimalPlan() {
  ...
  const constraints = nutrientKeys.map((key) => ({
    type: 'nutrient',
    key,
    coefficients: foods.map((food) => food.portionFactor * food[key]),
    value: targets[key],
  }));
...
function calcNutrientsForQuantities(quantities) {
  return foods.reduce((totals, food, index) => {
    ...
    totals.energy += quantity * food.portionFactor * food.energy;
